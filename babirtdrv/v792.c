...
int v792_segdata(unsigned long maddr){
  int wordcnt;

  wordcnt = 0;
  vread32(maddr+V792_OUTBUFF,(long *)(data+mp));
  mp += 2;
  segmentsize += 2;
  wordcnt++;
  ...
  if((data[mp-1] & V792_TYPE_MASK_S) == V792_HEADER_BIT_S){
    while(wordcnt < 34){
      /* vread32(maddr+V792_OUTBUFF+wordcnt*32,(long *)(data+mp)); */
      vread32(maddr+V792_OUTBUFF,(long *)(data+mp));
      mp += 2;
      segmentsize += 2;
      wordcnt++;
...
int v792_segdata2(unsigned long maddr){
  int wordcnt;
  long tdata;
...
int v792_segmod(short segid,unsigned long maddr){
  int tmp,wordcnt;
...
int v792_dmasegdata(unsigned long maddr, int rsize){
...
void v792_clear(unsigned long maddr){
  short sval;
