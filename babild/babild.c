...
void quit(void){
  ...
  if(vm) lfprintf(lfd, "babild: -- quit function --\n");

  if(runinfo.runstat != STAT_RUN_IDLE){
    if(vm) lfprintf(lfd, "babild: force quit during run\n");
  ...
  if(ebffd){
    if(vm) lfprintf(lfd, "babild: Close ebffd %d\n", ebffd);
    close(ebffd);
  }
  if(vm) lfprintf(lfd, "babild: Delete FIFO %s\n", EBFIFO);
  unlink(EBFIFO);
  
  if(comfd){
    if(vm) lfprintf(lfd, "babild: Close comfd %d\n",comfd);
    close(comfd);
  }
  if(esfd){
    if(vm) lfprintf(lfd, "babild: Close esfd %d\n",esfd);
    close(esfd);
  }
  if(ebdfd){
    if(vm) lfprintf(lfd, "babild: Close ebdfd %d\n",ebdfd);
    close(ebdfd);
  }
  if(infd){
    if(vm) lfprintf(lfd, "babild: Close infd %d\n", infd);
    close(infd);
  }
  ...
  if(vm) lfprintf(lfd, "babild: Exit\n");
...
int ebbufmalloc(int size){
  ...
  if(!(ebbuf.data = malloc(size))){
    if(vm) lfprintf(lfd, "ebbufmalloc: Can't malloc ebbuf size=%d\n", size);
    return 0;
  }
  if(vm) lfprintf(lfd, "babild: ebbuf malloc add=%p, size=%d\n", ebbuf.data, size);
...
int ssm_start(void){
  ...
  if(!(sock = ssmcon())){
    if(vm) lfprintf(lfd, "babild: ssm_start: Can't connect ssm\n");
    return 0;
  }
...
int xmlcomment(int mode){
  ...
  if(statcom.of){
    ...
    if((fd = popen(pcom, "r")) == NULL){
      if(vm) lfprintf(lfd, "Can't open statcom %s\n", pcom);
      flag = 0;
...
int reopen_file(){
  ...
  tgign ++;
  if(vm) lfprintf(lfd, "babild: reopen hdfiles number=%d\n", tgign);
  ...
  for(i=0;i<MAXHD;i++){
	...
	if(vm) lfprintf(lfd, "babild: Can't Re open %s (reopen_file)\n", fname);
      }else{
	mxfd[i] = tgigfd[i];
	if(vm) lfprintf(lfd, "babild: Re Open HD[%d]=%s\n", i, fname);
      }
    }
  }
...
int daq_start(){
  ...
  if(vm) lfprintf(lfd, "babild: daq_start runnumber=%d\n", daqinfo.runnumber);
  ...
  for(i=0;i<MAXHD;i++){
	...
	if(vm) lfprintf(lfd, "babild: Can't open %s (daq_start)\n", fname);
	mxfd[i] = NULL;
	return 0;
      }else{
	mxfd[i] = hdfd[i];
	if(vm) lfprintf(lfd, "babild: Open HD[%d]=%s\n", i, fname);
      }
    }
  }
...
int daq_nssta(void){
  ...
  if(escommand(ES_RUN_NSSTA)){
    ...
    if(vm) lfprintf(lfd, "babild: daq_nssta\n");
  }else{
    return 0;
  }
...
int daq_close(void){
  ...
  if(vm) lfprintf(lfd, "babild: daq stopped\n");
...
int update_esport(int nefn){
  ...
  if(daqinfo.babildes){
    if(nefn != efrc.efid){
      ...
      if(vm) lfprintf(lfd, "babild: esfd number = %d\n", esfd);
...
int store_daqinfo(void){
  ...
  if(!(fd = fopen(BABILDRC, "w"))){
    if(vm) lfprintf(lfd, "Can't open %s\n", BABILDRC);
    return 0;
  }
  fprintf(fd, "%s\n", daqinfo.runname);
  fprintf(fd, "%d\n", daqinfo.runnumber);
  ...
  fprintf(fd, "%d\n", daqinfo.efn);
  fprintf(fd, "%d\n", daqinfo.babildes);
  if(vm) lfprintf(lfd, "babild: store_daqinfo: babild.rc updated\n");
  fclose(fd);
...
int store_statcom(void){
  ...
  if(!(fd = fopen(STATCOMRC, "w"))){
    if(vm) lfprintf(lfd, "babild: can't open statcomrc = %d\n", STATCOMRC);
    return 0;
  }
...
int commain(void){
  ...
  case EB_SET_DAQINFO:
	...
	if(vm) lfprintf(lfd, "babild: ebsize changed %d\n", daqinfo.ebsize);
      }
      runinfo.runnumber = daqinfo.runnumber;
      store_eflist();
      store_mtlist();
      ...
      if(vm) lfprintf(lfd, "babild: now running, can't change daqinfo\n");
      com_rej(sock);
    }
    break;
  case EB_GET_RUNINFO:
  ...
  case EB_SET_RUNINFO:
    ...
    if(!runinfo.runstat){
      ...
      if(vm) lfprintf(lfd, "babild: now running, can't change runinfo\n");
      com_rej(sock);
    }
    break;
  case EB_SET_HEADER:
  ...
  case EB_SET_ENDER:
    ...
    if(runinfo.runstat == STAT_RUN_WAITSTOP){
      ...
      if(vm) lfprintf(lfd, "babild: now not waiting stop, can't change ender\n");
      com_rej(sock);
    }
    break;
  case EB_GET_EFNUM:
  ...
  case EB_SET_EFLIST:
    ...
    if(!runinfo.runstat){
      ...
      if(vm) lfprintf(lfd, "babild: now running, can't change eflist.\n");
      com_rej(sock);
    }
    break;
  case EB_GET_MTLIST:
  ...
  case EB_SET_MTLIST:
    ...
    if(!runinfo.runstat){
      ...
      if(vm) lfprintf(lfd, "babild: now running, can't change mtlist.\n");
      com_rej(sock);
    }
    break;
  case EB_GET_HDLIST:
  ...
  case EB_SET_HDLIST:
    ...
    if(!runinfo.runstat){
      ...
      if(vm) lfprintf(lfd, "babild: now running, can't change hdlist.\n");
      com_rej(sock);
    }
    break;
  case EB_GET_SSMINFO:
  ...
  case EB_SET_SSMINFO:
    ...
    if(!runinfo.runstat){
      ...
      if(vm) lfprintf(lfd, "babild: now running, can't change ssminfo.\n");
      com_rej(sock);
    }
    break;
  case EB_RUN_START:
  ...
  while(pt < ghd.blksize){
	...
	while(!newevt(buff+tpt, tsz, LEFN(thd.efn), loevtn, &flag)){
	  ...
	  if(flag == MEM_NMALLOC_EVTN_JUMP){
	    if(vm) lfprintf(lfd, "storeef: newevt warning1 flag = %d, evtn = %d first=%d last=%d\n", 
	...
	if(flag){
	  if(vm) lfprintf(lfd, "storeef: newevt warning2 flag = %d, evtn = %d first=%d last=%d\n", 
			  flag, loevtn, getfirstevtn(), getlastevtn());
	}
	...
	if(vm) lfprintf(lfd, "event data get from non EB member=%d\n", thd.efn);
      }
	...
	if(vm) lfprintf(lfd, "storeef: Fatal!! nevt can't sotre\n");
      }
...
int efrmain(void *opt){
  ...
  if(buff == NULL){
    if(vm) lfprintf(lfd, "efrmain[%d]: Cannot malloc buff size=%d\n", ebn, EB_EFBLOCK_BUFFSIZE);
    return 0;
  }

  /* Open FIFO FD */
  if((ebffdt = open(EBFIFO, O_RDWR)) == -1){
    if(vm) lfprintf(lfd, "efrmain[%d]: Can't open %s\n", ebn, EBFIFO);
    quit();
  }
  ...
  while(1){
    ...
    if((er = recv(sock, (char *)&len, sizeof(len), MSG_WAITALL)) < 1){
      if(vm) lfprintf(lfd, "efrmain[%d] : Error (or closed babies) in recv er=%d\n", ebn, er);
...
int ermain(void){
  ...
  case ES_RUN_START:
	...
	if(vm) lfprintf(lfd, "babild: already starting\n");
	ret = 0;
      }
    }else{
      if(vm) lfprintf(lfd, "babild: number of eflist.on = 0\n");
  ...
  case ES_RUN_STOP:
    ...
    if(runinfo.runstat){
      ...
      if(vm) lfprintf(lfd, "babild: not starting\n");
      ret = 0;
    }
    len = sizeof(ret);
    com_get(sock, (char *)&ret, len);
...
int main(int argc,char *argv[]){
  ...
  if(vm) lfprintf(lfd, "babild: Start PID=%d\n", pid);
  ...
  if(vm) lfprintf(lfd, "babild: Make FIFO %s\n",EBFIFO);

  /* Make command port */
  if((comfd = mktcpsock(EBCOMPORT)) == -1) quit();
  if(vm) lfprintf(lfd, "babild: comfd number = %d\n", comfd);
  if(comfd > maxfd) maxfd = comfd;

  /* Make EB data port */
  if((ebdfd = mktcpsock(ERRCVPORT)) == -1) quit();
  if(vm) lfprintf(lfd, "babild: ebfd number = %d\n", ebdfd);
  ...
  if(vm) lfprintf(lfd, "babild: infd number = %d\n", infd);
  ...
  if((fd = fopen(BABILDRC, "r"))){
    ...
    if(vm) lfprintf(lfd, "babild.rc: name=%s, runnumber=%d, ebsize=%d\n",
		  daqinfo.runname, daqinfo.runnumber, daqinfo.ebsize);
    fclose(fd);
  }else{
    if(vm) lfprintf(lfd, "no babild.rc\n");
  ...
  if(argefn){
    daqinfo.efn = argefn;
    if(vm) lfprintf(lfd, "babild: EFN is set %d by arg\n", daqinfo.efn);
  }
